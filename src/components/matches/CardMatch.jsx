...
const CardMatch = ({ match, onTeamClick }) => {
    ...
    if (isEditing) {
     ...
     return (
            ...
            <div className="flag" id="left">
                ...
                alt="bandera"
                className="bandera"
                onClick={() => handleTeamClick(match.idLocalTeam)}
                />
                <h1 className="nombrePais">{teamLcal.name}</h1>
            </div>
            {isEditing ? <input
                onChange={hanleInputResult}
                name="localGoal"
                value={result.localGoal}
                className="input-match-edit-teamL"
                type="number"
                placeholder={match.score.localGoal} /> : <span className="gol">{result.localGoal}</span>}
            <div className="datas">
            ...
            {isEditing ? <input
                onChange={hanleInputResult}
                name="visitingGoal"
                value={result.visitingGoal}
                className="input-match-edit-teamV"
                type="number"
                placeholder={match.score.visitingGoal} /> :
            ...
            <div className="flag" id="right">
                ...
                alt="bandera"
                className="bandera"
                onClick={() => handleTeamClick(match.idVisitingTeam)}
                />
